module RPM
  ...
  class Problem
    ...
    class OldPackage < Problem
      ...
      #
      # NOTE: Argument order is reversed to the one of `FileConflict`.
      def self.for(installing : Package | String, installed : Package | String,
                   key : String? = nil) : FileConflict
        if key.nil?
          if installing.is_a?(Package)
            key = installing.to_nevra
          else
            key = installing
          end
        end
        if installing.is_a?(Package)
          nevr1 = installing.to_nevr
        else
          nevr1 = installing
        end
        if installed.is_a?(Package)
          nevr2 = installed.to_nevr
        else
          nevr2 = installed
        end
        Problem.create(ProblemType::OLD_PACKAGE, nevr1, key, nevr2, nil, 0)
      end
    ...
    class Requires < Problem
      ...
      def self.for(package : Package, requires : String,
        ...
        key ||= package.to_nevra
        nevr = package.to_nevr
        Problem.create(ProblemType::REQUIRES, nil, key, nevr, requires, installed ? 1 : 0)
      end
      ...
      def self.for(package : String, requires : String,
                   installed : Bool, key : String? = nil) : Requires
        key ||= package
        Problem.create(ProblemType::REQUIRES, nil, key, package, requires, installed ? 1 : 0)
      end

      # Returns a pacakge which has missing Requires dependency
      def package
        {% if compare_versions(RPMVERSION_COMP, "4.9.0") < 0 %}
          pkg_nevr
        {% else %}
          alt_nevr
        {% end %}
    ...
    # Problem indicates that a Conflict dependency of a package is missing.
    class Conflict < Problem
      # Create CONFLICTS problem with given packages
      def self.for(package : Package, requires : String,
                   installed : Bool, key : String? = nil) : Conflicts
        key ||= package.to_nevra
        nevr = package.to_nevr
        Problem.create(ProblemType::CONFLICTS, nil, key, nevr, requires, installed ? 1 : 0)
      end
      ...
      def self.for(package : String, requires : String,
                   installed : Bool, key : String? = nil) : Conflicts
        key ||= package
        Problem.create(ProblemType::CONFLICTS, nil, key, package, requires, installed ? 1 : 0)
      end
    ...
    class Obsoletes < Problem
      ...
      def self.for(package : Package, requires : String,
        ...
        key ||= package.to_nevra
        nevr = package.to_nevr
        Problem.create(ProblemType::OBSOLETES, nil, key, nevr, requires, installed ? 1 : 0)
      end
      ...
      def self.for(package : String, requires : String,
                   installed : Bool, key : String? = nil) : Obsoletes
        key ||= package
        Problem.create(ProblemType::OBSOLETES, nil, key, package, requires, installed ? 1 : 0)
      end
    ...
    class Verify < Problem
      ...
      def self.for(package : Package, content : String, key : String? = nil) : Verify
        key ||= package.to_nevra
        nevr = package.to_nevr
        Problem.create(ProblemType::VERIFY, nevr, key, nil, content, 0)
      end

      # Create VERIFY problem with given packages
      def self.for(package : String, content : String, key : String? = nil) : Obsoletes
        key ||= package
        Problem.create(ProblemType::VERIFY, nevr, key, nil, content, 0)
      end
        ...
        {% begin %}
          case LibRPM.rpmProblemGetType(ptr)
            {% for m in [["BADARCH", "BadArch"], ["BADOS", "BadOS"],
                         ...
                         ["CONFLICT", "Conflict"], ["OBSOLETES", "Obsoletes"],
                         ["VERIFY", "Verify"]] %}
              {% tagname = m[0].id %}
              {% clsname = m[1].id %}
